...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if(args[0] === "delete") {
            ...
            return message.channel.send({ embeds: [client.util.successMsg(message.author.tag, "Configuration cleared")] })
        }
        ...
        if (pseudo === "cancel") return message.channel.send({ embeds: [client.util.successMsg(message.author.tag, "Cancelled command.")] })
        if (!pseudo) return message.channel.send({ embeds: [client.util.errorMsg(message.author.tag, "Time elapsed.")] })
        ...
        if (typeUser === "cancel") return message.channel.send({ embeds: [client.util.successMsg(message.author.tag, "Cancelled command.")] })
        if (!typeUser) return message.channel.send({ embeds: [client.util.errorMsg(message.author.tag, "Time elapsed.")] })
